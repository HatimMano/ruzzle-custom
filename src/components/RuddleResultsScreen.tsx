...
import type { RuddleResult } from "./RuddleGameScreen";
...
export default function RuddleResultsScreen({ mode, date, grid, validWords, result, onBack }: Props) {
  const [tab, setTab] = useState<Tab>("score");

  const bestWord = result.foundWords.reduce<string | null>(
    (best, w) => best === null || scoreForWord(w) > scoreForWord(best) ? w : best,
    null,
  );
  const pct = validWords.size > 0 ? Math.round((result.foundWords.length / validWords.size) * 100) : 0;
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: "0.2rem" }}>
          ...
          <p style={{ fontSize: "1.9rem", fontWeight: 700, color: "white", lineHeight: 1.1 }}>
            {result.score}
            <span style={{ fontSize: "1rem", color: "#475569", fontWeight: 600 }}> pts</span>
          </p>
          <p style={{ fontSize: "0.8rem", color: "#64748b" }}>
            {result.foundWords.length} mots · {fmtTime(result.elapsedSecs)}
          </p>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", paddingTop: "0.75rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: "1.8rem", fontWeight: 900, color: "white", fontVariantNumeric: "tabular-nums" }}>
                    {fmtTime(result.elapsedSecs)}
                  </div>
                  <div style={{ fontSize: "0.7rem", color: "#64748b", marginTop: "0.15rem" }}>durée</div>
                </div>
              </div>
          ...
          {tab === "classement" && <LeaderboardTab date={date} modeId={mode.id} />}
